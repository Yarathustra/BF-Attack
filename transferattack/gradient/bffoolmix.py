...
class BFFoolmix(Attack):
    """
    BF-Foolmix Attack Algorithm
    Boundary-based Foolmix with multiple boundary point sampling
    Combines Foolmix's dual-blending strategy with boundary fitting
    ...
    Arguments:
        ...
        num_boundary_points (int): number of boundary points to sample (default: 10).
        max_num_move (int): maximum number of moves to find boundary (default: 20).
        shrinkage_factor (float): shrinkage factor for boundary search (default: 0.8).
        initial_direction_std (float): standard deviation for initial direction sampling (default: 1.0).
        print_timing (bool): whether to print detailed timing statistics (default: True).

    Example script:
        python main.py --input_dir ./path/to/data --output_dir adv_data/bffoolmix/resnet50 --attack bffoolmix --model=resnet50
        python main.py --input_dir ./path/to/data --output_dir adv_data/bffoolmix/resnet50 --eval
    """

    def __init__(self, model_name, epsilon=16/255, alpha=1.6/255, epoch=10, decay=1., targeted=False,
                 random_start=False, norm='linfty', loss='crossentropy', device=None, attack='BF-Foolmix',
                 m=5, n=3, z=1, k=5, zeta=0.2, beta=1.0, gamma=0.1, print_timing=True,
                 use_amp=True, use_cache=False, grad_chunk_size=16,
    ...
    def forward(self, data, label, **kwargs):
        """
        The BF-Foolmix attack procedure
        """
        if self.targeted:
            assert len(label) == 2
            label = label[1]
        ...
        for t in range(self.epoch):
            ...
            for b in range(self.num_boundary_points):
                ...
                # Execute Foolmix gradient calculation at boundary point
                boundary_grad = self.calculate_foolmix_gradient(
                    data, boundary_delta, label,
                    iter_timing
                )

                grad_boundary.append(boundary_grad.detach())

            iter_timing['boundary_sampling_time'] = time.time() - start_time
            ...
            # Update momentum
            g_mix_norm = torch.norm(avg_boundary_grad, p=1, dim=(1, 2, 3), keepdim=True)
            momentum = self.decay * momentum + avg_boundary_grad / (g_mix_norm + 1e-8)
    ...
    def calculate_foolmix_gradient(self, data, delta, label, iter_timing):
        """
        Calculate Foolmix gradient at given delta position
        """
        x = data + delta
        ...
        if not self.targeted:
            ...
            for i in range(data.size(0)):
                if misclassified[i]:
                    f_topk = self.get_integrated_logits(x[i:i+1], top_k_indices[i])
                    omega_y = self.get_class_gradient(x[i:i+1], label[i:i+1])
                    omega_topk = self.get_integrated_gradient(x[i:i+1], top_k_indices[i])
                    d_direction = self.get_update_direction(f_topk, omega_y, omega_topk, label[i:i+1], x[i:i+1])
                    adjusted_delta = self.adjust_adversarial_example(delta[i:i+1], data[i:i+1], d_direction, self.alpha)
        ...
        start_time = time.time()
        g_lens = self.calculate_integrated_gradient_batch(x, P, L)
        iter_timing['integrated_gradient_time'] += time.time() - start_time

        # Calculate blended gradient with batch processing
        start_time = time.time()
        g_mix = self.calculate_average_blended_gradient_batch(x, P, g_lens, label)
    ...
    def print_timing_stats(self, timing_stats):
        ...
        print("\n" + "="*60)
        print("BF-FOOLMIX ATTACK TIMING STATISTICS")
        print("="*60)
        ...
        avg_times = {
            'boundary_sampling_time': 0, 'top_k_time': 0, 'misclassified_adjust_time': 0,
            'pixel_blocks_time': 0, 'other_labels_time': 0, 'integrated_gradient_time': 0,
            'blended_gradient_time': 0, 'momentum_update_time': 0, 'delta_update_time': 0,
            'total_iter_time': 0
        }
        ...
        print(f"Average Boundary sampling time: {avg_times['boundary_sampling_time']:.4f} seconds")
